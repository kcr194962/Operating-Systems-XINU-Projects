...
extern void allocatePageDirectories(int);
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	/*Create global page tables to map pages 0-4095 to physical 16 MB*/
	for(i = 0; i < 4; i++){
		if(get_frm(&frame_no) == SYSERR){
			return SYSERR;
		}
		frm_tab[frame_no].fr_status = FRM_MAPPED;
		frm_tab[frame_no].fr_pid = NULLPROC;
		frm_tab[frame_no].fr_type = FR_TBL;
		pt_entry = (FRAME0 + frame_no) * NBPG;

		for(j = 0; j< 1024; j++){
			pt_entry->pt_pres 	= 1;		/* page is present?		*/
			pt_entry->pt_write 	= 1;		/* page is writable?		*/
			pt_entry->pt_user 	= 0;		/* is use level protection?	*/
			pt_entry->pt_pwt 	= 0;		/* write through for this page? */
			pt_entry->pt_pcd 	= 0;		/* cache disable for this page? */
			pt_entry->pt_acc 	= 0;		/* page was accessed?		*/
			pt_entry->pt_dirty 	= 0;		/* page was written?		*/
			pt_entry->pt_mbz	= 0;		/* must be zero			*/
			pt_entry->pt_global = 1;		/* should be zero in 586	*/
			pt_entry->pt_avail 	= 0;		/* for programmer's use		*/
			pt_entry->pt_base 	= i * FRAME0 + j;

			pt_entry++;
		}
	}
	allocatePageDirectories(NULLPROC);
